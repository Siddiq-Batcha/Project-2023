...
void loop() {
 ...
 float weight = scale.get_units();
  ...
  if(weight < 15 ) {
    ...
    float finalVolume = (105.0/300.1)*(newWeight-27.0);
    ...
    pulsetenweight = finalCalculated+10;
        
    Serial.print(" Final Volume  ");
    Serial.print(pulsetenweight);
    Serial.println(" mL  ");

  
    totalInTank = pulsetenweight + tripWeight;
    totalInMessage = String(totalInTank);


    if(finalCalculated < 50.0){
      weightMessage = "Warning: Low Fuel " + pulsetenweight +" ml";
      Serial.println("LOW Fuel");
      digitalWrite(BUZPIN,HIGH);
    }
    else{
      weightMessage = " " + pulsetenweight +" ml";
...
String SendHTML(uint8_t tripStat,String weightMess,String totalInMess){
  ...
  ptr +="<h3> Total "+ totalInMess +" ml </h3>\n";
